...
const actions = [
  ...
  { label: "Add Card", description: "Link a card", to: "/add-card", icon: WalletCards },
  { label: "Cheques", description: "Cheque services", to: "/cheques", icon: FileCheck2 },
  { label: "Top Up", description: "Fund account", to: "/top-up", icon: CircleDollarSign },
  { label: "Withdraw", description: "Move funds out", to: "/withdraw", icon: ArrowDownToLine },
  { label: "Request", description: "Request payment", to: "/request", icon: HandCoins },
  { label: "Loans", description: "Lending options", to: "/loans", icon: Landmark },
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {actions.map(({ label, description, to, icon: Icon }) => (
          <Link
            key={label}
            to={to}
            className="group flex min-h-28 flex-col justify-between rounded-lg border border-slate-200 bg-white p-4 transition hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-sky-500 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-sky-700"
          >
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white sm:text-base">{label}</h3>

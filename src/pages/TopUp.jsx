  ...
  return (
    <div className="bg-white min-h-screen dark:bg-gray-800 shadow-lg p-6 transition-colors duration-300">
      ...
      <header className="flex items-center justify-between mb-8 w-full">
        ...
        <h2 className="text-lg sm:text-2xl md:text-3xl font-semibold text-sky-700 dark:text-white text-center flex-1">
          Top Up
        </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {paymentMethod === "Bank Transfer" && (
          <p className="p-3 bg-sky-50 dark:bg-gray-700 text-sky-700 dark:text-gray-200 rounded-lg">
            Please contact Pacific Horizon Bank customer service to complete this top-up via bank transfer.
          </p>
        )}

        {paymentMethod === "Mobile Payment" && (
          <p className="p-3 bg-sky-50 dark:bg-gray-700 text-sky-700 dark:text-gray-200 rounded-lg">
            Please contact Pacific Horizon Bank customer service to complete this top-up via mobile payment.
          </p>
        )}
        ...
        <div className="flex space-x-4">
          <button
            type="submit"
            className="flex-1 p-3 bg-gradient-to-br from-sky-500 to-sky-700 hover:from-sky-700 hover:to-sky-700 rounded-lg text-white font-semibold transition-all duration-200"
          >
            Top Up
          </button>
          <Link
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-md w-full p-6 text-center animate-fadeIn">
            <FaCreditCard className="text-4xl text-sky-700 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-gray-800 dark:text-white mb-2">
              Card Authentication Failed
            </h3>
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              We could not authenticate your card. Please check your details or try another payment method.
            </p>
            <button
              onClick={() => setShowModal(false)}
              className="px-6 py-2 bg-gradient-to-r from-sky-700 to-sky-700 rounded-lg text-white font-semibold hover:from-sky-700 hover:to-sky-800 transition-all"
            >
              Close
            </button>
          </div>
